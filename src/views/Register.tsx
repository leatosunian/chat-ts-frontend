...
const Register = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if( !inputValues?.name ){
            handleAlert({ msg: 'Debes ingresar tu nombre', error: true });
            return;
        } 
        if( inputValues?.confirmPass !== inputValues?.password ){
            handleAlert({ msg: 'Las contraseñas no coinciden', error: true });
            return;
        }
        if( !inputValues?.phone ){
            handleAlert({ msg: 'Debes ingresar tu teléfono', error: true });
            return;
        } 
        ...
        if(response.data.response_data === 'USER_EXISTS'){
            handleAlert({error: true, msg: 'El usuario ya existe'});
            return;
        }
